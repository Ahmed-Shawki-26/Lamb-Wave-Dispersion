...
class Lamb:
    ...
    def __init__(
        ...
        if c_L <= 0 or c_S <= 0:
            raise ValueError(f"c_L and c_S must be positive (got c_L={c_L}, c_S={c_S}).")
        if c_L <= c_S:
            raise ValueError(
                f"c_L must be greater than c_S (got c_L={c_L}, c_S={c_S})."
            )
        ...
        self._safe_material = (material or "").replace("/", "_").replace("\\", "_").strip()

        # Solve the dispersion equations.

        sym = self._solve_disp_eqn(
            function=self._symmetric, nmodes=nmodes_sym, c=c_S, label="S"
        )

        antisym = self._solve_disp_eqn(
                    ...
                    if not np.isnan(x_1) and not np.isnan(x_2):
                        if np.sign(x_1) != np.sign(x_2):
                            bisection = scipy.optimize.bisect(
                                f=function, a=vp_1, b=vp_2, args=(fd,)
                            )
                            ...
                            if np.abs(
                                function(bisection, fd)
                            ) < 1e-2 and not np.isclose(bisection, c):
    ...
    def animate_displacement(
        ...
        if mode[0] == "S" and int(mode[1:]) < self.nmodes_sym:
            interp = self.vp_sym[mode]
        elif mode[0] == "A" and int(mode[1:]) < self.nmodes_antisym:
            interp = self.vp_antisym[mode]
        else:
            raise ValueError(
                'mode not recognized. Mode must be "Sn" or '
        ...
        # Clamp fd to the interpolation range to avoid out-of-bounds errors.
        fd_clamped = np.clip(fd, float(interp.x[0]), float(interp.x[-1]))
        vp = interp(fd_clamped)
        ...
        ax.set_title(r"Mode $\mathregular{" + mode[0] + "_" + mode[1:] + "}$")
        ax.text(
            0.5,
            0.05,
            f"fd = {np.around(fd, 1)} kHz × mm",
            ha="center",
            va="center",
        ...
        anim = matplotlib.animation.FuncAnimation(
            fig, animate, init_func=init, frames=time, interval=speed, blit=True
        )

        if save_gif:
            plt.rcParams["animation.convert_path"] = Lamb.magick_path
            anim.save(
                f"results/Mode_{mode}_fd_{int(fd)}_animation.gif",
                writer="imagemagick",
                extra_args="convert",
            )
    ...
    def plot(
        ...
        for mode, arr in result.items():
            ...
            if cutoff_frequencies:
                add_cutoff_freqs(ax, mode, arrow_dir, y_max, self.c_L, self.c_S)
    ...
    def plot_phase_velocity(
        ...
        if modes == "symmetric":
            ...
            raise ValueError('modes must be "symmetric", "antisymmetric"or "both".')
        ...
        if save_img:
            fig.savefig(
                f"results/Phase Velocity - {self.d * 1e3} mm {self._safe_material} plate.png",
                bbox_inches="tight",
            )
    ...
    def plot_group_velocity(
        ...
        if modes == "symmetric":
            self.plot(
                ax, self.vg_sym, max_sym, cutoff_frequencies, "up", plt_kwargs=sym_style
            )
        elif modes == "antisymmetric":
            self.plot(
                ax,
            ...
            max_ = max(max_sym, max_antisym)
            self.plot(
                ax, self.vg_sym, max_, cutoff_frequencies, "up", plt_kwargs=sym_style
            )
            self.plot(
                ax,
                self.vg_antisym,
        ...
        if save_img:
            fig.savefig(
                f"results/Group Velocity - {self.d * 1e3} mm {self._safe_material} plate.png",
                bbox_inches="tight",
            )

        return fig, ax

    def plot_wave_number(
        self,
        modes="both",
        save_img=False,
        sym_style={"color": "blue"},
        ...
        if save_img:
            fig.savefig(
                f"results/Wave Number - {self.d * 1e3} mm {self._safe_material} plate.png",
                bbox_inches="tight",
            )

        return fig, ax

    def plot_wave_structure(
        self,
        mode,
        nrows,
        ncols,
        ...
        fig.suptitle(r"Mode $\mathregular{" + mode[0] + "_" + mode[1:] + "}$")
        ...
        for ax, freq in zip(axs.flatten(), fd):
            ...
            # Clamp freq to the interpolation range to avoid
            # out-of-bounds errors from scipy interp1d.
            fd_min, fd_max_mode = float(interp.x[0]), float(interp.x[-1])
            freq_clamped = np.clip(freq, fd_min, fd_max_mode)
            vp = interp(freq_clamped)
        ...
        if save_img:
            fig.savefig(
                f"results/Wave Structure - {self.d * 1e3} mm "
                f"{self._safe_material} plate - Mode {mode}.png",
                bbox_inches="tight",
            )
    ...
    def save_results(self):
        ...
        if self._safe_material:
            filename = f"{self._safe_material} plate - {self.d * 1e3} mm.txt"
        else:
        ...
        write_txt(self.vp_sym, self.vp_antisym, "Phase Velocity", filename, header)
        write_txt(self.vg_sym, self.vg_antisym, "Group Velocity", filename, header)
        write_txt(self.k_sym, self.k_antisym, "Wavenumber", filename, header)
