...
Then use this repository from its folder (clone or download and extract).
...
## CFRP Toolkit (in this repository)

This repo includes extra scripts for CFRP quasi-isotropic laminates and a desktop GUI:

| File | Description |
|------|-------------|
| **lamb_wave_gui.py** | Desktop GUI: Setup (CLT presets, c_L/c_S/c_R), dispersion plots, wave structure, animation, Abaqus parameters, export. No coding required. |
| **run_gui.bat** | Double-click to run the GUI (Windows). Sets `PYTHONIOENCODING=utf-8` and runs `lamb_wave_gui.py`. |
| **dispersion_analysis.py** | Script: CLT + dispersion curves + Abaqus parameter derivation (e.g. IM7/8552). |
| **animate_lamb_wave.py** | Script: animated quiver plots for A0 and S0 at a chosen fd (GIF + HTML). |

**Run from this folder** (the repo root):

```bash
# GUI (recommended)
python lamb_wave_gui.py
# or on Windows: run_gui.bat

# Scripts
python dispersion_analysis.py
python animate_lamb_wave.py
```

Outputs (plots, TXT, GIF, HTML, Abaqus exports) are saved under `results/`, with a subfolder per material (e.g. `results/T300_5208_1.0mm_cL6780/`).
